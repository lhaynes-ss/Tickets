/**
    ...
    Country: 'US' -- US | CA
    mapping file: @udw_clientsolutions_cs.samsung_ads_data_share/analytics/custom/vaughn/paramount/20241118_p_us.csv
**/
...
-- set variables
SET start_dt = '2024-11-11'; -- 'YYYY-MM-DD'
SET end_dt = '2024-11-17'; -- 'YYYY-MM-DD'
SET country = 'US'; -- US | CA
...
CREATE TEMP TABLE raw_place_mapping (
    ...
    ,creative_id INT
    ,delete_country2 VARCHAR(556)
    ,line_item_id INT
    ,delete_impressions INT
);
COPY INTO raw_place_mapping 
FROM @udw_clientsolutions_cs.samsung_ads_data_share/analytics/custom/vaughn/paramount/20241118_p_us.csv
...
CREATE TEMP TABLE place_mapping AS (
    ...
    WHERE
        CAST(camp_start AS DATE) <= CAST($end_dt AS DATE) -- camp_start >= window_end
        AND CAST(camp_end AS DATE) >= CAST($start_dt AS DATE) -- camp_end >= window_start
);
...
CREATE TEMP TABLE ip_psid_map AS (
    ...
    FROM (
        SELECT
            m.vpsid
            ,IFNULL(fact.ipv6_pii_virtual_id, fact.ip_pii_virtual_id) AS vip
            ,m.vtifa
            ,ROW_NUMBER() OVER (PARTITION BY IFNULL(fact.ipv6_pii_virtual_id, fact.ip_pii_virtual_id) ORDER BY TO_TIMESTAMP(FLOOR(timestamp)::INT) DESC) AS rn
        FROM trader.log_delivery_raw_without_pii fact
            JOIN UDW_LIB.VIRTUAL_PSID_TIFA_MAPPING_V m ON GET(fact.samsung_tvids_pii_virtual_id, 0) = m.vtifa
        WHERE 
            fact.udw_partition_datetime BETWEEN TO_TIMESTAMP($report_start_date,'YYYYMMDDHH') AND TO_TIMESTAMP($report_end_date,'YYYYMMDDHH')
...
CREATE TEMP TABLE cd_ccc_imp_and_click AS (
    SELECT DISTINCT 
        ...
        c.campaign_end_date,
        TO_TIMESTAMP(FLOOR(timestamp)::INT) AS timing,
        MIN(TO_TIMESTAMP(FLOOR(timestamp)::INT)) OVER(PARTITION BY vtifa, event) AS first_day
    FROM trader.log_delivery_raw_without_pii AS fact
        JOIN creative_map c USING (campaign_id, creative_id)
    WHERE 
        DATE_TRUNC('day', TO_TIMESTAMP(FLOOR(timestamp)::INT)) BETWEEN TO_TIMESTAMP($report_start_date,'YYYYMMDDHH') AND TO_TIMESTAMP($report_end_date,'YYYYMMDDHH')
        AND event IN (
...
CREATE TEMP TABLE page_visitors AS (
    SELECT 
        TO_TIMESTAMP(FLOOR(a.timestamp)::INT) AS timing
        ,segment_id
        ,c.vtifa
    FROM trader.log_delivery_raw_without_pii a  
        JOIN ip_psid_map c ON IFNULL(a.ipv6_pii_virtual_id, a.ip_pii_virtual_id) = c.vip
    WHERE 
        event = 'segment'
...
CREATE TEMP TABLE app_usage AS (
    ...
    FROM data_tv_acr.fact_app_usage_session_without_pii AS fact
        LEFT JOIN UDW_LIB.VIRTUAL_PSID_TIFA_MAPPING_V AS m ON m.vpsid = fact.psid_pii_virtual_id
    WHERE 
...
-- ---------------------
SELECT
    'Paramount+_Q224 Initiatives' AS campaign_name,
    campaign_id,
    placement_id,
    creative_id,
    creative_name,
...
CREATE TEMP TABLE CD AS (
    SELECT DISTINCT 
        ...
        creative_id, 
        campaign_id, 
        DATE_TRUNC('day', TO_TIMESTAMP(FLOOR(timestamp)::INT)) AS partition_date
    FROM trader.log_delivery_raw_without_pii AS fact
    WHERE 
