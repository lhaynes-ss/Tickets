/**
    ...
    Country: 'US' -- US | CA
    mapping file:  @udw_clientsolutions_cs.samsung_ads_data_share/analytics/custom/vaughn/pluto/20241118_pluto_us.csv
**/
...
FROM @udw_clientsolutions_cs.samsung_ads_data_share/analytics/custom/vaughn/pluto/20241118_pluto_us.csv
...
CREATE TEMP TABLE exposure_log AS (
    SELECT
        ...
        ,(TO_CHAR(CAST(udw_partition_datetime AS DATE), 'yyyymmdd')) AS expose_date
        ,udw_partition_datetime
        ,creative_id
        ,campaign_id
    FROM trader.log_delivery_raw_without_pii
    WHERE 
        (TO_CHAR(CAST(udw_partition_datetime AS DATE), 'yyyymmdd')) BETWEEN $report_start_date AND $report_end_date 
    ...
    FROM ( 
        ...
        FROM (SELECT * FROM exposure_log UNION SELECT * FROM click_log ) a
            JOIN (
                ...
                a.vtifa = b.vtifa 
                AND TO_DATE(a.expose_date, 'YYYYMMDD') <= b.date_first_open
            )
    )
    WHERE row_num = 1
    GROUP BY 1,2
